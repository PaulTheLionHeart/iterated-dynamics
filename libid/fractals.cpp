...
**************************************************************************/
/*
static int subtype, special;
static Complex g_a, g_a2, g_aa3, g_b, g_t2, g_t3, g_v, g_q;

int ArtMatrixfp_per_pixel() // Art Matrix Cubic (taken from the original Fortran)
{
    Complex z, temp;
...
//	  period_level = FALSE;			            // no periodicity checking
    ...
    else if (subtype == 1 || subtype == 2)      // CCIN or CFIN
    {
        g_a = g_q;                              // A = T
                                                // find B = T + 2*T*T*T
        temp = g_q.CCube();                     // B = T*T*T
        if (subtype == 1)
            g_b = temp + temp + g_q;            // B = B * 2 + T
        else
        {
            g_b = (temp - g_q) * 2;             // B = B * 2 - 2 * T
            g_a2 = g_a + g_a;
        }
    }
...
**************************************************************************/
/*
int ArtMatrixfpOrbit()                          // Art Matrix Cubic (taken from the original Fortran)
{
    Complex z, temp;
    z.x = g_old_z.x;
    z.y = g_old_z.y;
