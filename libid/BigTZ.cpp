...
static  BigComplex aBig, bBig, c1Big, c2Big, caa3Big, cbBig, zBig, z1Big, z2Big, z3Big, z4Big, zdBig, ztBig, qBig;
    ...
    switch (subtype)
	    ...
	    */
	    ...
	    case 106:					// More Fractals, 5th order N/Mset
	        if (!juliaflag)
		        {
		        zBig->x = -qBig->x * (double)(degree - 2) / (double)degree + g_params[2];
		        zBig->y = -qBig->y * (double)(degree - 2) / (double)degree + g_params[3];
		        }
	        break;
...
//    	case 59:					// More Newton Msets, 5th order Newton Mset
	        ...
	        *zBig = zBig->CInvert();
	        zBig->x = zBig->x + g_params[2];
	        zBig->y = zBig->y + g_params[3];
	        break;
...
//	        if (!juliaflag)
	    ...
	    case 128:					// Flarium 16, Newton Variations: z = z - (z*z*z*z*z-1)/(5*z*z*z*z) + c; [5th Order Nova]
	        zBig->x = 1.0 + g_params[2];
	        zBig->y = 0.0 + g_params[3];
	        break;
...
int	BigRunTierazonFunctions(int subtype, BigComplex *zBig, BigComplex *qBig)
    {
    switch (subtype)
	    {
	    case 0:						// Mandelbrot
	    ...
	    *zBig = zBig->CSqr() + *qBig;
	    ...
	    case 90:					// Polynomials, z=z+c [quad]
	        ...
	        *zBig = *zBig + *qBig;
	        ...
	        zdBig = *zBig - z1Big;
	        d = zdBig.CSumSqr();
	        return (d < MINSIZE || d > g_magnitude_limit);
	    ...
	    *zBig = *zBig * *zBig;
	    ...
	    case 116:					// Ramon, z = (c/sin(z))^2
	        ...
	        *zBig = zBig->CSqr();
	        zdBig = *zBig - z1Big;
	        return (zdBig.CSumSqr() < MINSIZE || zBig->CSumSqr() > g_magnitude_limit);
	    ...
	    case 133:					// Flarium 28, Derbyshire / Newton: z=z - (z*z*z-1)/(3*z*z)+c; [Nova-Mandelbrot-MultiFract]
	        ...
	        if (g_color_iter >= g_max_iterations)
		        ...
		        *zBig = *zBig + 1;
		        i = 0;
		        d = 1;
		        while (d > MINSIZE && i++ < g_max_iterations)
		            {
		            z1Big = *zBig;
