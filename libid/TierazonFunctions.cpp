...
static Complex a, b, c, c1, c2, cb, caa3, t, Sqr, z, z1, z2, z3, z4, zd, zt, q;
...
//	        return FractintBailoutTest(z);
    ...
    */
	    ...
	    case 116:					// Ramon, z = (c/sin(z))^2
	        ...
	        *z = z->CSqr();
	        zd = *z - z1;
	        return (zd.CSumSqr() < MINSIZE || z->CSumSqr() > g_magnitude_limit);
...
bool	FractintBailoutTest(Complex *z)
    ...
    switch (g_bail_out_test)
        ...
        case bailouts::Real:
            TempSqr.x = sqr(z->x);
	        return (TempSqr.x >= g_magnitude_limit);
	
        case bailouts::Imag:
            TempSqr.y = sqr(z->y);
	        return (TempSqr.y >= g_magnitude_limit);

        case bailouts::Or:
            TempSqr.x = sqr(z->x);
	        TempSqr.y = sqr(z->y);
	        return (TempSqr.x >= g_magnitude_limit || TempSqr.y >= g_magnitude_limit);
...
int tierazonfp_per_pixel()
    ...
    {
        case fractal_type::NEWTONNOVA:
            subtype = 2;
            break;
        case fractal_type::TALIS:
            subtype = 4;
            break;
        case fractal_type::NEWTONPOLYGON:
            subtype = 31;
            break;
        case fractal_type::NEWTONFLOWER:
            subtype = 35;
            break;
        ...
        case fractal_type::NEWTONAPPLE:
            subtype = 55;
            break;
        case fractal_type::NEWTONCROSS:
            subtype = 59;
            break;
        case fractal_type::NEWTONVARIATION:
            subtype = 05;
            break;
        case fractal_type::QUARTET:
            int version = (int) g_params[3];
            switch (version)
            {
            case 0:
                subtype = 81;
